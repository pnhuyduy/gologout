...
var spawnArgs = (options2, payload2, args = []) => {
...
var startChrome = async (dir) => {
  try {
    const filePref = import_node_path.default.join(dir, "Default", "Preferences");
    const prefs = generator_exports.getNewFingerprint(payload, options);
    import_fs_extra.default.ensureFileSync(filePref);
    import_fs_extra.default.writeJSON(filePref, prefs);
    ...
    const child = (0, import_child_process.execFile)(executePath, params);
...
(async () => {
  const userDataDirA = import_node_path.default.join(import_node_path.default.resolve(), "profiles", "abc");
