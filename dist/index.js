...
var import_simple_proxy_agent = __toESM(require("simple-proxy-agent"));
...
var checkTimezone = async (proxy) => {
  ...
  if (proxy.mode === "http") {
    agent = new import_hpagent.HttpsProxyAgent({
      keepAlive: true,
      keepAliveMsecs: 1e3,
      maxSockets: 256,
      maxFreeSockets: 256,
    ...
    agent = new import_simple_proxy_agent.default(proxyAgent);
  }
...
var getNewFingerprint = (payload, options) => {
  ...
  const randomDeviceMemory = deviceMemoryList[randomInt(0, deviceMemoryList.length)];
  newGologinConfig.deviceMemory = randomDeviceMemory;
  const hardwareConcurrencyList = [4, 6, 12, 24];
  const randomHardwareConcurrency = hardwareConcurrencyList[randomInt(0, hardwareConcurrencyList.length)];
  ...
  if ((0, import_os.platform)() === "darwin") {
    ...
    newGologinConfig.userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.5672.92 Safari/537.36";
